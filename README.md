...
Caldera will start on http://127.0.0.1:8888

Default Username: red
...
```
After a few seconds, the Sandcat agent should appear in Caldera under the Agents tab.
...
* Type in DNP3 in the search to view all possible DNP3 commands from the Caldera DNP3 Plugin
...
Or any custom abilities you create yourself
...
Filter examples:
* DNP3 protcool filter: `dnp3`
* TCP port filter (default DNP3 port): `tcp.port == 20000`
...
* Ensure no other process is using the port: `lsof -i :20000`

If Wireshark shows no traffic:
* Ensure loopback capture is enabled
* Try filtering with: `tcp`
